...
function addDeclarationToNgModule(options) {
  return host => {
    ...
    const componentPath =
      ...
      (options.flat
        ? ""
        : core_1.strings.dasherize(options.name) + "-container/") +
      core_1.strings.dasherize(options.name) +
      ".container";
    const relativePath = find_module_1.buildRelativePath(
      modulePath,
      componentPath
    );
    const classifiedName = core_1.strings.classify(`${options.name}Container`);
